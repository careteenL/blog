...
- 从根元素`a`开始遍历，并放入队列第一个位置
  - 指针指向第一个位置，取队列第二个元素`a`，得到`a`的儿子们，将`a/b、a/c`依次放入队列中
  - 指针指向第二个位置，取队列第三个元素`a/b`的儿子们，将`a/b/d`放入队列中
  - 指针指向第三个位置，取队列第四个元素`a/c`的儿子们，将`a/c/e`放入队列中
  - 指针指向第四个位置，取队列第五个元素`a/b/d`的儿子们，发现没有，跳过
  - 指针指向第五个位置，取队列第六个元素`a/c/e`的儿子们，发现没有，跳过
  - 遍历结束
