...
- [写入文件](#写入文件)
- [拷贝文件](#拷贝文件)
  - [编码问题1：爬虫爬取的网页是GBK](#编码问题1：爬虫爬取的网页是GBK)
  - [编码问题2：文件从GBK转为UTF8会出现BOM头问题](#编码问题2：文件从GBK转为UTF8会出现BOM头问题)
  - [问题3：拷贝一份超出当前内存特别大的文件](#问题3：拷贝一份超出当前内存特别大的文件)
- [递归创建目录](#递归创建目录)
  - [同步方式实现](#同步方式实现)
  - [异步方式实现](#异步方式实现)
- [递归删除目录](#递归删除目录)
  - [同步深度优先先序删除多级目录](#同步深度优先先序删除多级目录)
  - [异步深度优先串行先序删除多级目录](#异步深度优先串行先序删除多级目录)
  - [异步深度优先并行先序删除多级目录](#异步深度优先并行先序删除多级目录)
  - [异步(Promise)深度优先并行先序删除多级目录](#异步(Promise)深度优先并行先序删除多级目录)
  - [异步(Async)深度优先并行先序删除多级目录](#异步(Async)深度优先并行先序删除多级目录)
  - [同步广度优先删除多级目录](#同步广度优先删除多级目录)
  - [异步广度优先删除多级目录](#异步广度优先删除多级目录)
...
![](./assets/wide.desc.png)

如上图，现在存在一个多级目录关系`a->b->d、a->c->e`，广度优先遍历需要一个队列和一个指针

- 从根元素`a`开始遍历，并放入队列第一个位置，指针指向第一个位置
  - 指针指向第二个位置，取队列第二个元素`a`，得到`a`的儿子们，将`a/b、a/c`依次放入队列中
  - 指针指向第三个位置，取队列第三个元素`a/b`的儿子们，将`a/b/d`放入队列中
  - 指针指向第四个位置，取队列第四个元素`a/c`的儿子们，将`a/c/e`放入队列中
  - 指针指向第五个位置，取队列第五个元素`a/b/d`的儿子们，发现没有，跳过
  - 指针指向第六个位置，取队列第六个元素`a/c/e`的儿子们，发现没有，跳过
  - 遍历结束
